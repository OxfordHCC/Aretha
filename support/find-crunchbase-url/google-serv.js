...
const createDriver = () => {
    ...
    findCompany = (cname) => {
        ...
        return Promise.delay(1500 + 1000 * Math.random())
            ...
            .then(() => driver.wait(until.elementLocated(By.css('cite.iUh30')), 60000))
            .then((card) => {
                console.log('==> got it! ');
                return driver.findElements(By.css('cite.iUh30'));
            }).then((elements) => {
                console.log('els > ', elements.length);
    ...
    save_db = () => {
        ...
        console.info('cache path ', cache_path, '---', database);
        fs.writeFileSync(cache_path, JSON.stringify(database));
    },
    run_server = (driver) => {
        app.use(cors());
