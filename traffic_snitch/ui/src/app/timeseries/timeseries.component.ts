...
export class TimeseriesComponent implements AfterViewInit, OnChanges {
	...
	@Input() devices : { [mac:string]: string } = {}; // Device[];
	...
	legend_els: string[];
	legend_to_name: { [id:string]: string };
	legend_to_cat: { [company:string]: string };
	legend_colours: { [company:string]: string };
	...
	render() {
		...
		const impacts_arr = this.impacts_arr,			
			...
			stack_keys = _.uniq(_.flatten(new Array(...impacts_arr.map(v => Object.keys(v.impacts)))));

		stack_keys.sort();
		
		this.legend_els = stack_keys;  // testing overflow .concat(stack_keys).concat(stack_keys);
		this.legend_to_cat = this.company_to_info ? this.legend_els.reduce((obj, x) => { 
			obj[x] = this.company_to_info[x] && this.company_to_info[x].typetag || undefined;
			return obj;
		}, {}) : undefined;
		this.legend_colours = this.legend_els.reduce((obj,x) => {
			obj[x] = persistentColor(x);
			return obj;
		}, {});
		this.legend_to_name = this.byDestination ? undefined : _.mapValues(this.devices, (v) => v.name);
